...
func handleRequest(_ request: Request, bleManager: BLEManager) -> Response? {
    ...
    case "read_characteristic":
        ...
        guard let deviceAddress = request.params?["device_address"]?.value as? String,
              let serviceUUID = request.params?["service_uuid"]?.value as? String,
              let charUUID = request.params?["char_uuid"]?.value as? String else {
            return Response.error(
                id: request.id,
                code: ErrorCode.invalidParams,
                message: "Missing required params: device_address, service_uuid, char_uuid"
            )
        }
        ...
        Task { @MainActor in
            do {
                let data = try await bleManager.readCharacteristic(address: deviceAddress, serviceUUID: serviceUUID, charUUID: charUUID)
    ...
    case "write_characteristic":
        ...
        guard let deviceAddress = request.params?["device_address"]?.value as? String,
            ...
            return Response.error(
                id: request.id,
                code: ErrorCode.invalidParams,
                message: "Missing required params: device_address, service_uuid, char_uuid, value"
            )
        }
        ...
        Task { @MainActor in
            do {
                try await bleManager.writeCharacteristic(address: deviceAddress, serviceUUID: serviceUUID, charUUID: charUUID, data: data, withResponse: withResponse)
    ...
    case "subscribe":
        ...
        Task { @MainActor in
            do {
                try await bleManager.subscribe(address: deviceAddress, serviceUUID: serviceUUID, charUUID: charUUID)
    ...
    case "unsubscribe":
        ...
        Task { @MainActor in
            bleManager.unsubscribe(address: deviceAddress, serviceUUID: serviceUUID, charUUID: charUUID)
