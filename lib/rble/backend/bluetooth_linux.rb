  ...
  module Backend
    ...
    class BluetoothLinux < Base
      ...
      def initialize
        ...
        # Connection tracking
        @connected_devices = {}  # device_address => true
        @device_services = {}    # device_address => [service_data, ...]
        @connection_objects = {} # device_address => Connection instance
      ...
      # Check if the Linux helper binary is available
      def self.available?
      ...
      def ensure_subprocess
        ...
        raise HelperNotFoundError, HELPER_PATH unless File.exist?(HELPER_PATH)
      ...
      # @param device_identifier [String] Device MAC address (from scanning)
      ...
      # @param device_identifier [String] Device MAC address
      # @return [void]
      def disconnect_device(device_identifier)
        ...
        begin
          send_request('disconnect', { address: device_identifier }, timeout: 5)
      ...
      def device_path_for_address(address, adapter: nil)
        # On Linux, address is a MAC address - validate format
        unless address =~ /^[0-9A-F]{2}(:[0-9A-F]{2}){5}$/i
          raise ConnectionError, "Invalid Linux device address '#{address}'. " \
            "Use MAC address format (e.g., 'AA:BB:CC:DD:EE:FF')."
        end
        address.upcase
      end
      ...
      # @param char_identifier [String] Format: "device_address:service_uuid:char_uuid"
      ...
      def read_characteristic(char_identifier, timeout: 30)
        device_address, service_uuid, char_uuid = parse_char_identifier(char_identifier)
      ...
      def handle_device_discovered(params)
        ...
        device = Device.new(
          address: params['address'], # On Linux, MAC address is the identifier
          name: params['name'],
          rssi: params['rssi'],
          ...
          address_type: params['address_type'] || 'public'
        )
      ...
      def handle_notification(params)
        ...
        identifier = "#{device_address}:#{service_uuid}:#{char_uuid}"
      ...
      def handle_disconnected(params)
        ...
        connection = @state_mutex.synchronize do
          @connected_devices.delete(address)
          @device_services.delete(address)
          @connection_objects.delete(address)
        end
      ...
      # @param identifier [String] Format: "device_address:service_uuid:char_uuid"
      # @return [Array<String>] [device_address, service_uuid, char_uuid]
      # @raise [ArgumentError] if identifier is invalid
      def parse_char_identifier(identifier)
        parts = identifier.split(':')
        # MAC address has 5 colons, so we expect 8 parts total (6 for MAC + 2 for UUIDs)
        # However, UUIDs also have dashes, not colons, so let's be smarter about this
        # Format: "AA:BB:CC:DD:EE:FF:service-uuid:char-uuid"
        raise ArgumentError, "Invalid characteristic identifier: #{identifier}" unless parts.length >= 8

        # First 6 parts are the MAC address
        device_address = parts[0..5].join(':')
        service_uuid = parts[6]
        char_uuid = parts[7]

        [device_address, service_uuid, char_uuid]
      end
